...
// Utilidades para ejes "bonitos" en fl_chart
double _niceStep(double span) {
  if (span <= 0 || span.isNaN || span.isInfinite) return 1;
  final desired = span / 6; // ~6 divisiones visibles
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
=======
  List<FlSpot> series(int n) =>
      List.generate(n + 1, (i) { final x = duration * i / n; return FlSpot(x, temp(x)); });
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
=======
      t0 = p(t0C, t0); ta = p(taC, ta); k = p(kC, k).abs(); duration = p(dC, duration).clamp(1, 1e6);
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
=======
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          ...
          Chip(label: Text('t½ ≈ ${tHalf.toStringAsFixed(2)} min')),
          Chip(label: Text('t95% ≈ ${t95.toStringAsFixed(2)} min')),
          ActionChip(avatar: const Icon(Icons.local_cafe, size: 16), label: const Text('Bebida'), onPressed: (){ setState((){ t0=60; ta=25; k=0.25; duration=30; t0C.text='60'; taC.text='25'; kC.text='0.25'; dC.text='30'; tMarker=0; }); }),
          ActionChip(avatar: const Icon(Icons.memory, size: 16), label: const Text('CPU'), onPressed: (){ setState((){ t0=70; ta=25; k=0.35; duration=20; t0C.text='70'; taC.text='25'; kC.text='0.35'; dC.text='20'; tMarker=0; }); }),
          ActionChip(avatar: const Icon(Icons.ac_unit, size: 16), label: const Text('Exterior frío'), onPressed: (){ setState((){ ta=10; taC.text='10'; tMarker=0; }); }),
        ]),
        const SizedBox(height: 12),
        SizedBox(
          height: 200,
          child: Card(
            ...
            child: Padding(
              ...
              child: LineChart(LineChartData(
                ...
                lineBarsData: [
                  LineChartBarData(
                    spots: series(400), isCurved: true, color: Colors.indigo, barWidth: 3,
                    dotData: const FlDotData(show: true),
                    belowBarData: BarAreaData(
                      show: true,
                      gradient: LinearGradient(
        ...
        Wrap(crossAxisAlignment: WrapCrossAlignment.center, spacing: 12, runSpacing: 8, children: [
          ...
          OutlinedButton.icon(onPressed: _reset, icon: const Icon(Icons.replay), label: const Text('Reiniciar')),
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButton<double>(
              value: _speed,
              items: const <double>[0.25, 0.5, 1.0, 2.0, 4.0, 8.0]
                  .map((double v) => DropdownMenuItem<double>(value: v, child: Text('${v}x')))
                  .toList(),
              onChanged: (v) => setState(() => _speed = v ?? 1.0),
            )
          ]),
        ...
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: Slider(value: tMarker.clamp(0, duration), min: 0, max: duration, onChanged: (v){
            setState(()=>tMarker=v);
          }))
        ]),
        // Fórmulas (aprovecha el espacio en blanco)
...
=======
  ...
  double temp(double t) => ta + (t0 - ta - q/(k==0?1:k)) * math.exp(-k * t) + (k==0?0:q/k);
  List<FlSpot> series(int n) => List.generate(n+1, (i){final x = duration*i/n; return FlSpot(x, temp(x));});

  void apply() {
    double p(TextEditingController c, double f) => double.tryParse(c.text.replaceAll(',', '.')) ?? f;
    setState(() {
      t0 = p(t0C, t0); ta = p(taC, ta); P = p(pC, P); C = p(cC, C).abs(); hA = p(haC, hA).abs(); duration = p(dC, duration).clamp(1, 1e6);
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
      if (tMarker > duration) tMarker = duration;
    });
  }
...
=======
    final steady = ta + (hA>0 ? P/hA : 0);
    final currentT = temp(tMarker);
    final minY = [temp(0), temp(duration), ta, steady].reduce((a,b)=>math.min(a,b)) - 2;
    final maxY = [temp(0), temp(duration), ta, steady].reduce((a,b)=>math.max(a,b)) + 2;
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
    return Padding(
      padding: const EdgeInsets.all(16),
      child: LayoutBuilder(
                ...
                child: Card(
                  ...
                  child: Padding(
                    ...
                    child: LineChart(LineChartData(
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          spots: series(400), isCurved: true, color: Colors.blue, barWidth: 3,
                          dotData: const FlDotData(show: true),
                          belowBarData: BarAreaData(
                            show: true,
                            gradient: LinearGradient(
                      ...
                      extraLinesData: ExtraLinesData(horizontalLines: [
                        HorizontalLine(y: ta, color: Colors.teal, dashArray: const [6,4], label: HorizontalLineLabel(show: true, labelResolver: (_)=>'Tₐ ${ta.toStringAsFixed(1)}°C')),
                        HorizontalLine(y: steady, color: Colors.orangeAccent, dashArray: const [6,4], label: HorizontalLineLabel(show: true, labelResolver: (_)=>'T∞ ${steady.toStringAsFixed(1)}°C')),
                      ], verticalLines: [
...
=======
            return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [left, const SizedBox(width: 16), Expanded(flex: 1, child: rightContent)]);
          } else {
            // Rehacer la columna izquierda sin Expanded y permitir scroll
            final leftSmall = Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Wrap(spacing: 12, runSpacing: 12, children: [
                numField('T₀ (°C)', t0C), numField('Tₐ (°C)', taC), numField('P (W)', pC), numField('C (J/°C)', cC), numField('hA (W/°C)', haC), numField('Duración (min)', dC),
                FilledButton.icon(onPressed: apply, icon: const Icon(Icons.check), label: const Text('Aplicar')),
                Chip(label: Text('T∞ ≈ ${steady.toStringAsFixed(2)} °C')),
...
=======
  const ServerRackWidget({super.key, required this.ambient, required this.temperature, this.steady, this.powerWatts});
  final double ambient;      // T_a
  final double temperature;  // T(t)
  final double? steady;      // T_infty opcional
  final double? powerWatts;  // P opcional
...
=======
class _ServerRackWidgetState extends State<ServerRackWidget> with SingleTickerProviderStateMixin {
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
<<<<<<< HEAD
      return Color.lerp(Colors.blue.shade400, Colors.red.shade400, ratio) ??
          Colors.orange;
=======
      return Color.lerp(Colors.blue.shade400, Colors.red.shade400, ratio) ?? Colors.orange;
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
    }
...
=======
          Row(children: [
            Icon(Icons.dns, color: tempColor(widget.temperature)),
            const SizedBox(width: 8),
            Expanded(child: Container(height: 6, decoration: BoxDecoration(color: Colors.grey.shade800, borderRadius: BorderRadius.circular(6)),
              child: Align(
                alignment: Alignment.centerLeft,
                child: FractionallySizedBox(
                  widthFactor: 0.7,
                  child: Container(height: 6, decoration: BoxDecoration(color: tempColor(widget.temperature), borderRadius: BorderRadius.circular(6))),
                ),
              ),
            )),
            const SizedBox(width: 8),
          ...
          // 6 units with fans and grills
          Expanded(
            child: Column(children: List.generate(6, (i) => Expanded(
              child: Padding(
                padding: const EdgeInsets.symmetric(vertical: 2),
                child: _ServerUnit(controller: _c, color: tempColor(widget.temperature - i)),
              ),
            ))),
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
          ),
...
=======
                Text('Tₐ ${widget.ambient.toStringAsFixed(1)}°C', style: const TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(width: 8),
                Text('T ${widget.temperature.toStringAsFixed(1)}°C', style: TextStyle(color: tempColor(widget.temperature), fontSize: 12, fontWeight: FontWeight.bold)),
                if (widget.steady != null) ...[
                  const SizedBox(width: 8),
                  Text('T∞ ${widget.steady!.toStringAsFixed(1)}°C', style: const TextStyle(color: Colors.orangeAccent, fontSize: 12)),
                ]
              ],
            ),
          )
...
=======
        return Stack(children: [
          Align(
            alignment: Alignment(-1.0, inY * 2 - 1),
            child: Icon(Icons.arrow_forward_ios, size: 18, color: Colors.lightBlueAccent.withValues(alpha: 0.9)),
          ),
          Align(
            alignment: Alignment(1.05, outY * 2 - 1),
            child: Transform.rotate(
              angle: math.pi,
              child: Icon(Icons.arrow_forward_ios, size: 18, color: Colors.orangeAccent.withValues(alpha: 0.9)),
            ),
          ),
        ]);
      },
...
=======
      child: Row(children: [
        ...
        Expanded(
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: List.generate(3, (i) => _Fan(controller: controller, color: color, speed: 1.0 + i * 0.2)),
          ),
        ),
        // LEDs column (compact to avoid tiny-height overflows)
        Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _MiniLed(color: Colors.lightGreenAccent, controller: controller, phase: 0.1),
            _MiniLed(color: Colors.cyanAccent, controller: controller, phase: 0.5),
            _MiniLed(color: Colors.deepPurpleAccent, controller: controller, phase: 0.8),
          ],
        )
      ]),
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
=======
  const _Fan({required this.controller, required this.color, required this.speed});
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
  final AnimationController controller;
  final Color color;
  final double speed;
...
=======
      ...
      child: Stack(children: [
        Container(
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: RadialGradient(colors: [color.withValues(alpha: 0.15), Colors.black], stops: const [0.3, 1.0]),
            border: Border.all(color: Colors.grey.shade800, width: 1),
          ),
        ),
        Center(
          child: RotationTransition(
            turns: Tween<double>(begin: 0, end: speed).animate(controller),
            child: Icon(Icons.settings, size: 20, color: Colors.white.withValues(alpha: 0.9)),
          ),
        ),
      ]),
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
=======
  const _BlinkLed({required this.color, required this.controller, required this.phase});
  final Color color; final AnimationController controller; final double phase;
  @override
  Widget build(BuildContext context) {
    return FadeTransition(
      opacity: TweenSequence([
        TweenSequenceItem(tween: Tween(begin: 0.2, end: 1.0), weight: 50),
        TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.2), weight: 50),
      ]).animate(CurvedAnimation(parent: controller, curve: Interval(phase, (phase + 0.9).clamp(0.0, 1.0)))),
      child: Container(width: 10, height: 10, decoration: BoxDecoration(color: color, shape: BoxShape.circle, boxShadow: [BoxShadow(color: color.withValues(alpha: 0.6), blurRadius: 6)])),
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
    );
  }
}
...
=======
  const _MiniLed({required this.color, required this.controller, required this.phase});
  final Color color; final AnimationController controller; final double phase;
  @override
  Widget build(BuildContext context) {
    return FadeTransition(
      opacity: Tween(begin: 0.4, end: 1.0).animate(CurvedAnimation(parent: controller, curve: Interval(phase, (phase + 0.5).clamp(0.0, 1.0)))),
      child: Container(width: 5, height: 5, decoration: BoxDecoration(color: color, shape: BoxShape.circle, boxShadow: [BoxShadow(color: color.withValues(alpha: 0.6), blurRadius: 3)])),
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
    );
  }
}
...
=======
  const DataCenterView({super.key, required this.ambient, required this.temperature, this.steady, this.powerWatts});
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
...
class _DataCenterViewState extends State<DataCenterView>
    with SingleTickerProviderStateMixin {
=======
class _DataCenterViewState extends State<DataCenterView> with SingleTickerProviderStateMixin {
>>>>>>> e14f450cd7a86212bc4b8f0da9cd70f199a847e4
  late final AnimationController _c;
  @override
  void initState() {
        ...
        return Stack(children: [
          ...
          Positioned(
            ...
            child: Row(children: const [
              Icon(Icons.ac_unit, size: 14, color: Colors.lightBlueAccent),
              SizedBox(width: 4),
              Text('Cold Aisle', style: TextStyle(fontSize: 12, color: Colors.lightBlueAccent)),
            ]),
          ),
          Positioned(
            right: 8, top: 8,
            child: Row(children: const [
              Icon(Icons.local_fire_department, size: 14, color: Colors.orangeAccent),
              SizedBox(width: 4),
              Text('Hot Aisle', style: TextStyle(fontSize: 12, color: Colors.orangeAccent)),
            ]),
          ),
          // Racks
          Positioned.fill(
